@if ($supportedLocales && count($supportedLocales) > 1)
    ...
    @if (setting('language_switcher_display', 'dropdown') == 'dropdown')
        ...
        <div class="dropdown">
            <button class="btn btn-secondary dropdown-toggle d-inline-flex align-items-center btn-language" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
            ...
            <ul class="dropdown-menu language_bar_chooser bg-dark text-white {{ Arr::get($options, 'class') }}">
                        ...
                        <li>
                            <a rel="alternate" hreflang="{{ $localeCode }}" href="{{ $showRelated ? Language::getLocalizedURL($localeCode) : url($localeCode) }}">
                                <div class="d-flex align-items-center px-3">
                                    ...
                                    @if (Arr::get($options, 'lang_name', true) && ($languageDisplay == 'all' || $languageDisplay == 'name'))<span class="ms-2 text-white">{{ $properties['lang_name'] }}</span>@endif
                                </div>
                            </a>
                        </li>
                    @endif
